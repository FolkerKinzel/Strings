...
internal sealed class Utf8Validator
{
    ...
    /// /> characters long is UTF-8 text. The method takes the Byte Order Mark (BOM) into
    /// account.</summary>
    /// <param name="stream">The <see cref="Stream" /> to examine.</param>
    /// <param name="count">The number of characters to examine at least. If <paramref name="count"
    /// /> is larger than <paramref name="stream" /> has data, <paramref name="stream" />
    /// is examined until EOF. Passing a negative number to this parameter lets the method
    /// examine the whole <paramref name="stream" /> from the current position until EOF.
    /// Passing <c>0</c> lets the method examine the BOM only.</param>
    /// <param name="leaveOpen"> <c>false</c> to let the method close the <paramref name="stream"
    /// />, <c>true</c> otherwise.</param>
    /// <returns> <c>true</c> if <paramref name="stream" /> might be UTF-8 text, otherwise
    /// <c>false</c>. If the method finds a UTF-8 BOM, it always returns <c>true</c>. If
    /// <paramref name="count" /> is <c>0</c> and there's no BOM, it returns <c>false</c>.</returns>
    ...
    /// /> at least. If <paramref name="count" /> is larger than <paramref name="stream"
    /// /> has data, <paramref name="stream" /> is examined until EOF. Passing a negative
    /// number to this parameter lets the method examine from the current position until
    /// EOF. <c>0</c> is not allowed.</param>
    /// <param name="leaveOpen"> <c>false</c> to let the method close the <paramref name="stream"
    /// />, <c>true</c> otherwise.</param>
    /// <returns> <c>true</c> if the examined sequence in <paramref name="stream" /> is valid
    /// UTF-8 text, otherwise <c>false</c>.</returns>
    /// <exception cref="ArgumentNullException"> <paramref name="stream" /> is <c>null</c>.</exception>
